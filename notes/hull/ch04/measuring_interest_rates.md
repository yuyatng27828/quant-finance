...
## Simple Interest
Interest is no reinvested - you earn interest only on the principal
$$Total Value = P(1 + RT)$$

## Compound Interest
Interest is reinvested each period - you earn interest on interest
$$A = P(1 + \frac{R}{m})^{mT}$$

## Continuous Compounding
Interest compounds infinitely often &rarr; smooth exponential growth
$$A = Pe^{RT}$$
...
To compare rates with different compounding frequencies, use the effective annual rate (EAR):
$$R_{eff} = (1 + \frac{R_{nominal}}{m})^m - 1$$

## Discount Factors
Discount factors translate a future cash flow into present value:
$$d(t) = e^{-R_ct}$$
If continuously compounded rate = 5%, 1-year discount factor = $$e^{-0.05}$$ = 0.9512
